    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                if (!personJson) throw new Error(`Person with ID ${personId} not found`);
                ...
                const visitsJson = allVisitsData.filter(visit => visit.personid == personId);
            ...
            <header className="py-4 bg-white border-b w-full">
                <div className="flex justify-between items-center py-2">
                    <div className="flex items-center px-[10px] py-[5px] whitespace-nowrap">
                        ...
                        <span className="text-sm font-medium text-gray-900 ml-1">{personData.person.person_id}</span>
                        <span className="text-gray-200 mx-3">|</span>
                        <span className="text-sm text-gray-400">Gender</span>
                        <span className="text-sm font-medium text-gray-900 ml-1">{genderCodes[personData.person.gender_concept_id]}</span>
                        <span className="text-gray-200 mx-3">|</span>
                        <span className="text-sm text-gray-400">Birth(Year)</span>
                        <span className="text-sm font-medium text-gray-900 ml-1">{personData.person.year_of_birth}</span>
                    </div>
                    ...
                    <div className="w-full">
                        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                            ...
                            <ChartCard
                                ...
                                tableContent={<BarChartTableView data={(personStatistics?.topTenDrug || []).map(item => ({ name: item.label, count: item.value }))} domainKey="drug" />}
                            >
                                <BarChartWrapper data={
                                    (personStatistics?.topTenDrug || [])
                            ...
                            <ChartCard
                                ...
                                tableContent={<BarChartTableView data={(personStatistics?.topTenCondition || []).map(item => ({ name: item.label, count: item.value }))} domainKey="condition" />}
                            >
                                <BarChartWrapper data={(personStatistics?.topTenCondition || []).map(item => ({ name: item.label, count: item.value }))} />
                            </ChartCard>
                            ...
                            <ChartCard
                                ...
                                tableContent={<BarChartTableView data={(personStatistics?.topTenProcedure || []).map(item => ({ name: item.label, count: item.value }))} domainKey="procedure" />}
                            >
                                <BarChartWrapper data={(personStatistics?.topTenProcedure || []).map(item => ({ name: item.label, count: item.value }))} />
                            </ChartCard>
                            ...
                            <ChartCard
                                ...
                                tableContent={<BarChartTableView data={(personStatistics?.topTenMeasurement || []).map(item => ({ name: item.label, count: item.value }))} domainKey="measurement" />}
                            >
                                <BarChartWrapper data={(personStatistics?.topTenMeasurement || []).map(item => ({ name: item.label, count: item.value }))} />
                            </ChartCard>
                        </div>
                    </div>
                ) : (
