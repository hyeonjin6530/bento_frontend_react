...
export default function CategoryTree() {
    ...
    return (
        <aside className="min-w-64 max-w-72 bg-white border-r border-gray-200 h-screen overflow-y-auto p-4 pb-12 flex flex-col">
            <h2 className="text-xl font-semibold text-blue-700 mb-4 border-b border-gray-200 pb-2 flex items-center gap-2">
                테이블 및 컬럼
            </h2>

            <input
                type="text"
                className="mb-4 w-full px-3 py-2 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm"
                placeholder="검색할 항목을 입력하세요."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
            />

            <ul className="list-none p-0 w-full pb-8">
                {filteredCategories.map((cat) => (
                    <li key={cat.table} className="mb-0 border-b border-gray-100">
                        <button
                            className="flex items-center w-full px-2 py-2 rounded-lg font-semibold text-gray-900 hover:bg-blue-50 transition-colors"
                        ...
                        >
                            ...
                            <span className="flex-1 text-left ml-2 text-[13px]">
                <span
                ...
                />
                        ...
                        {cat.columns && isOpen(cat.table) && (
                            <ul className="ml-4 mt-1 border-l border-gray-100 pl-2 pb-2">
