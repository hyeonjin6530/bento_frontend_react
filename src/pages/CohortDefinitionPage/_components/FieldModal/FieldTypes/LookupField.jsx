...
export default function LookupField({
    ...
    const selectedGrouped = useMemo(() => {
        return selectedItems.reduce((acc, item) => {
            ...
            if (!acc[key]) acc[key] = { conceptName: item.target_concept_name, snuhs: [] };
            acc[key].snuhs.push({
    ...
    return (
        <div className="flex flex-col h-full min-h-0 w-full overflow-hidden">
            <div className="flex w-full h-full px-4">
                {/* 좌측 테이블 */}
                <div className="basis-2/3 min-w-0 shrink-0 border border-gray-200 rounded-lg flex flex-col min-h-0 mr-4">
                    {/* 검색 영역 */}
                    <div className="bg-gray-50 px-4 pb-0 pt-2 border-b border-gray-200 rounded-t-lg">
              ...
              <span className="text-sm font-medium text-gray-700">
                {apiData.length} / {totalCount} 건
                            ...
                            <div className="flex items-center gap-2">
                                <p>
                                    ...
                                    <span className="text-xs text-gray-600">검색 후 엔터를 눌러주세요</span>
                                </p>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="통합 검색"
                                        className="w-48 h-8 pl-8 pr-4 py-2 border border-gray-300 rounded text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder:text-xs"
                    ...
                    {/* 테이블 헤더 */}
                    <div className="flex items-center px-2 py-2 bg-gray-100 border-b border-gray-200 text-xs font-medium text-gray-700 text-center">
                    ...
                    {/* 열별 검색창 */}
                    <div className="flex items-center px-2 py-2 bg-white border-b border-gray-200 text-xs text-gray-700 text-center">
                        <div className="w-[5%]" />
                        <div className="w-[15%] px-0.5">
                            <input
                                type="text"
                                className="w-full px-2 py-1 border border-gray-300 rounded text-xs focus:ring-1 focus:ring-blue-500 focus:outline-none"
                        ...
                        {isLoading ? (
                            ...
                            apiData.map((item) => (
                                <div
                                    ...
                                    className="flex items-start px-2 py-2 border-b border-gray-100 hover:bg-gray-50 transition-colors text-sm cursor-pointer text-center"
                                    onClick={() => toggleItem(item)}
                                    onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && toggleItem(item)}
                                    role="button"
                                    tabIndex={0}
                                >
                                    ...
                                    <div
                                        className="w-[15%] px-1.5 font-medium text-gray-900 line-clamp-3 whitespace-normal break-words self-center"
                                        title={item.target_concept_id}
                                    >
                                        {item.target_concept_id}
                                    </div>
                                    <div
                                        className="w-[25%] px-1.5 text-gray-700 text-left self-center"
                                    ...
                                    <div
                                        className="w-[15%] px-1.5 text-gray-700 line-clamp-3 whitespace-normal break-words self-center"
                ...
                {/* 우측 선택 영역 */}
                <div className="basis-1/3 min-w-0 shrink-0 border border-gray-200 rounded-lg flex flex-col overflow-hidden">
                    <div className="flex flex-col bg-gray-50 px-4 pt-3 pb-1 border-b border-gray-200 rounded-t-lg">
                        <div className="flex flex-row items-center justify-between">
                            <h5 className="font-medium text-gray-800 text-sm">선택된 항목</h5>
                            <div className="flex flex-col text-right">
                                <p className="text-xs text-gray-600">Concept ID {Object.keys(selectedGrouped).length}개</p>
                                <p className="text-xs text-gray-600">SNUH ID {selectedItems.length}개</p>
                            </div>
                        </div>
                        <p>
                            <span className="text-xs text-red-600">*</span>
                            <span className="text-[11px] text-gray-600">SNUH ID가 아닌 Concept ID를 기준으로 적용됩니다.</span>
                        </p>
                    </div>

                    <div className="flex-1 overflow-y-auto p-2 space-y-1 min-h-[300px] overflow-x-hidden">
                        {selectedItems.length === 0 ? (
                            ...
                            Object.entries(selectedGrouped).map(([conceptId, group]) => (
                                <div key={conceptId} className="border border-blue-200 bg-blue-50 rounded p-2 text-sm space-y-2">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center min-w-0 gap-2">
                                            ...
                                            <div className="text-xs text-blue-600 truncate min-w-[25px] max-w-[50px]" title={conceptId}>
                                                {conceptId}
                                            </div>
                                            <div className="font-medium text-blue-800 truncate max-w-[200px] pr-1" title={group.conceptName}>
                                                {group.conceptName}
                                            </div>
                                        </div>
                                        <button
                                            ...
                                            onClick={() => removeSelected({ target_concept_id: conceptId })}
                                        >
                                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <ul className="list-disc pl-4">
                                                {group.snuhs.map((snuh, idx) => (
                                                    <div key={idx} className="flex items-center">
                                                        <li className="text-xs text-blue-900 truncate w-[50px]" title={`${snuh.snuhId}`}>
                                                            • {snuh.snuhId}
                                                        </li>
                                                        <li className="text-xs text-blue-900 truncate w-[200px]" title={`${snuh.snuhName}`}>
                                                            {snuh.snuhName}
                                                        </li>
                                                    </div>
                                                ))}
