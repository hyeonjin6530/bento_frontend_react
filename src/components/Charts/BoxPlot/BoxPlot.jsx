...
const BoxPlot = ({ data, colors = ["#4595EC", "#FF6B6B", "#FFD166", "#06D6A0", "#9D8DF1"] }) => {
...
const drawChart = useCallback(() => {
    if (!data || !data.result || data.result.length === 0 || !svgContainerRef.current) return;
    ...
    const tooltip = d3.select(document.body).append("div")
        .attr("class", "boxplot-tooltip fixed hidden bg-black/80 text-white px-3 py-2 rounded-md text-xs pointer-events-none");
    ...
    data.result.forEach(cohort => {
      cohort.values.forEach((groupValues, groupIndex) => {
        ...
        const groupData = { group: groupNames[groupIndex], target: getTargetName(targetId), color: colors[data.result.findIndex(d => (d.cohortId ?? d.personId) === targetId) % colors.length], outliers: [] };
