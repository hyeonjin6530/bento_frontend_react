...
const BoxPlot = ({
  ...
  const drawChart = useCallback(() => {
          ...
          color:
            colors[
              data.result.findIndex(
                (d) => (d.cohortId ?? d.personId) === targetId,
              ) % colors.length
            ],
          outliers: [],
        };
    ...
    boxplotData.forEach((d) => {
      ...
      const allTargetsWidth =
        targets.length * xTargetScale.bandwidth() +
        (targets.length - 1) * xTargetScale.step() * xTargetScale.padding();
      const offset = (groupWidth - allTargetsWidth) / 2;

      const boxX =
        xGroup + offset + xTarget + (xTargetScale.bandwidth() - boxWidth) / 2;
