...
export default function GroupDonutChartWrapper({ data = [] }) {
    ...
    const { processedData, legendData } = useMemo(() => {
        const pData = data?.map(cohort => ({ name: cohort.cohortName, data: cohort.data })) || [];
    ...
    return (
        <div className="flex flex-col items-center justify-center w-full">
            <div className="w-full overflow-x-auto">
                <div className="flex flex-row justify-center items-start gap-2 px-4" style={{ minWidth: '400px' }}>
                    {processedData.map(chart => (
                        <div key={chart.name} className="flex flex-col items-center w-[200px]">
                            <DonutChart
                                data={chart.data}
                            ...
                            />
                            <span className="break-words text-center text-sm font-medium text-gray-600" style={{ maxWidth: '160px' }}>
                                {chart.name}
                            </span>
                        </div>
                    ))}
